...
| `ha_database_vip` | `""` | Virtual IP for the HA controller (for example `10.0.0.100/24`). DRBD Reactor floats an IPaddr2 resource with the active controller. The IP is stored as `Aux/ha_database_vip` so `client_install` discovers it on subsequent runs. Defaults to `/24` if no CIDR is given. When set, the role installs the IPaddr2 OCF resource agent via `linbit.drbd_reactor.resource_agents_upstream` (narrowed to just `IPaddr2`) on combined nodes where it is not already present. |
